...
- **Committable + auto-sync** — graph persists in `.grasp-it/`, can be committed to git, auto-detects staleness via git diff
...
1. **Skill analysis**: Run `/understand` on a sample project → verify `.grasp-it/knowledge-graph.json` is generated with correct schema
