...
**Issue:** [#159](https://github.com/akimkelar/Grasp-It/issues/159) — Frequently seeing output limit exceeded
...
**Location:** `grasp-it-plugin/skills/understand/compute-batches.mjs`

**Invocation:** `node <SKILL_DIR>/compute-batches.mjs $PROJECT_ROOT [--changed-files=<path>]`

**Input:** `$PROJECT_ROOT/.grasp-it/intermediate/scan-result.json`

**Output:** `$PROJECT_ROOT/.grasp-it/intermediate/batches.json`

### Dependencies

Added to `grasp-it-plugin/package.json`:

- `graphology` (~10KB)
- `graphology-communities-louvain` (~30KB)

Reuses `@grasp-it/core`'s `TreeSitterPlugin` and `PluginRegistry` (already imported by `extract-structure.mjs`).
...
1. Load `scan-result.json` from this repo's `.grasp-it/` directory (if absent, generate via `/understand --full`).
...
Reads `.grasp-it/intermediate/scan-result.json`, writes
`.grasp-it/intermediate/batches.json`.
...
Load `.grasp-it/intermediate/batches.json` (produced by
...
> Output: write to
> `$PROJECT_ROOT/.grasp-it/intermediate/batch-<batchIndex>.json`
...
- If `nodeCount ≤ 60` AND `edgeCount ≤ 120`: write ONE file to
  `.grasp-it/intermediate/batch-<batchIndex>.json`. Done.
  Skip to Step E.
...
`.grasp-it/intermediate/batch-<batchIndex>-part-<k>.json`.
...
New file: `grasp-it-plugin/skills/understand/test_compute_batches.test.mjs` (Vitest).
...
- [ ] Run `/understand --full` on this repo (Grasp-It itself):
...
11. Add `graphology` + `graphology-communities-louvain` to `grasp-it-plugin/package.json`.
