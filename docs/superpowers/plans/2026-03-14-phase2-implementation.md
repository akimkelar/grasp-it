...
export function loadGraph(
  ...
  const graphPath = path.join(baseDir, '.grasp-it', 'knowledge-graph.json');
...
**Context:** This is the first Claude Code skill command. `/understand-chat` provides in-terminal Q&A using the knowledge graph. As a Claude Code skill, it needs: (1) a skill markdown file that Claude loads, (2) a context-builder that extracts relevant graph context for a user query, (3) the prompt template that combines context + query. The skill reads the persisted `.grasp-it/knowledge-graph.json` and uses the active Claude session for LLM — no separate API call needed.
...
import type { KnowledgeGraph } from '@grasp-it/core';
...
```bash
pnpm install && pnpm --filter @grasp-it/skill test
```
...
import { SearchEngine } from '@grasp-it/core';
import type { KnowledgeGraph, GraphNode, GraphEdge, Layer } from '@grasp-it/core';
...
1. Read the knowledge graph file at `.grasp-it/knowledge-graph.json` in the current project root
...
```typescript
import { SearchEngine } from '@grasp-it/core';
import type { KnowledgeGraph, SearchResult } from '@grasp-it/core';
...
2. Uses `buildChatContext` + `formatContextForPrompt` from `@grasp-it/core` (or inline the same logic since the skill package uses core)
...
pnpm --filter @grasp-it/core test
pnpm --filter @grasp-it/skill test
pnpm dev:dashboard
```
...
- `pnpm --filter @grasp-it/skill build` — Build skill package
- `pnpm --filter @grasp-it/skill test` — Run skill tests
...
9. **All tests pass**: `pnpm --filter @grasp-it/core test && pnpm --filter @grasp-it/skill test`
