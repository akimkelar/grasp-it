...
**Architecture:** Five changes (C5 → C4 → C3 → C1+C2) applied in rollout order — lowest risk first. All changes are to prompt/skill markdown files in `grasp-it-plugin/skills/understand/`. No TypeScript source changes required.
...
**Files:**
- Modify: `grasp-it-plugin/skills/understand/SKILL.md` (Phase 6, lines 330–362)
...
Write the following Node.js script to `$PROJECT_ROOT/.grasp-it/tmp/ua-inline-validate.js`:
...
Execute it:
```bash
node $PROJECT_ROOT/.grasp-it/tmp/ua-inline-validate.js \
...
**Files:**
- Modify: `grasp-it-plugin/skills/understand/SKILL.md` (Phase 4, around line 188–196)
...
**Files:**
- Modify: `grasp-it-plugin/skills/understand/SKILL.md` (Phase 5, lines 257–270)
- Modify: `grasp-it-plugin/skills/understand/tour-builder-prompt.md` (input schema)
...
```bash
git add grasp-it-plugin/skills/understand/SKILL.md \
        grasp-it-plugin/skills/understand/tour-builder-prompt.md
...
**Files:**
- Modify: `grasp-it-plugin/skills/understand/SKILL.md` (Phase 2, lines 104–117)
- Modify: `grasp-it-plugin/skills/understand/file-analyzer-prompt.md` (add quick reference section)
...
```bash
git add grasp-it-plugin/skills/understand/SKILL.md \
        grasp-it-plugin/skills/understand/file-analyzer-prompt.md
...
```bash
cat > $PROJECT_ROOT/.grasp-it/tmp/ua-file-analyzer-input-<batchIndex>.json << 'ENDJSON'
{
...
**Files:**
- Modify: `grasp-it-plugin/skills/understand/SKILL.md` (Phase 0, Phase 1, Phase 2)
...
```bash
node -e "const p = require('./grasp-it-plugin/package.json'); console.log(p.version)"
```
...
Update each file:
- `grasp-it-plugin/package.json`: `"version": "1.2.2"`
...
```bash
grep -r '"version"' grasp-it-plugin/package.json .claude-plugin/marketplace.json .claude-plugin/plugin.json .cursor-plugin/plugin.json
```
...
```bash
VERSION=$(node -e "const p = require('./grasp-it-plugin/package.json'); console.log(p.version)")
...
- `importMap` is present in `scan-result.json` (check `.grasp-it/intermediate/` before cleanup, or add a temporary debug log)
