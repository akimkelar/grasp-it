...
### Core package
- Create: `grasp-it-plugin/packages/core/src/ignore-filter.ts` — parse .understandignore, merge with defaults, filter paths
- Create: `grasp-it-plugin/packages/core/src/ignore-generator.ts` — generate starter .understandignore by scanning project
- Create: `grasp-it-plugin/packages/core/src/__tests__/ignore-filter.test.ts` — filter tests
- Create: `grasp-it-plugin/packages/core/src/__tests__/ignore-generator.test.ts` — generator tests
- Modify: `grasp-it-plugin/packages/core/src/index.ts` — export new modules
- Modify: `grasp-it-plugin/packages/core/package.json` — add `ignore` dependency

### Agents & skills
- Modify: `grasp-it-plugin/agents/project-scanner.md` — add Layer 2 filtering step
- Modify: `grasp-it-plugin/skills/understand/SKILL.md` — add Phase 0.5
...
Run:
```bash
cd grasp-it-plugin && pnpm add --filter @grasp-it/core ignore
```

- [ ] **Step 2: Verify it was added**

Run: `grep ignore grasp-it-plugin/packages/core/package.json`
...
**Files:**
- Create: `grasp-it-plugin/packages/core/src/ignore-filter.ts`
- Create: `grasp-it-plugin/packages/core/src/__tests__/ignore-filter.test.ts`
...
describe("IgnoreFilter", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(testDir, ".grasp-it"), { recursive: true });
  });
  ...
  describe("createIgnoreFilter with user .understandignore", () => {
    it("reads patterns from .grasp-it/.understandignore", () => {
      writeFileSync(
        join(testDir, ".grasp-it", ".understandignore"),
        "# Exclude tests\n__tests__/\n*.test.ts\n"
      );
      const filter = createIgnoreFilter(testDir);
    ...
    it("merges .grasp-it/ and root .understandignore", () => {
      writeFileSync(
        join(testDir, ".grasp-it", ".understandignore"),
        "__tests__/\n"
      );
...
Run: `pnpm --filter @grasp-it/core test -- --run src/__tests__/ignore-filter.test.ts`
 ...
 * 2. .grasp-it/.understandignore (if exists)
...
export function createIgnoreFilter(projectRoot: string): IgnoreFilter {
  ...
  // Layer 2: .grasp-it/.understandignore
  const projectIgnorePath = join(projectRoot, ".grasp-it", ".understandignore");
...
```bash
git add grasp-it-plugin/packages/core/src/ignore-filter.ts grasp-it-plugin/packages/core/src/__tests__/ignore-filter.test.ts
...
**Files:**
- Create: `grasp-it-plugin/packages/core/src/ignore-generator.ts`
- Create: `grasp-it-plugin/packages/core/src/__tests__/ignore-generator.test.ts`
...
Run: `pnpm --filter @grasp-it/core test -- --run src/__tests__/ignore-generator.test.ts`
Expected: FAIL — module not found
...
```bash
git add grasp-it-plugin/packages/core/src/ignore-generator.ts grasp-it-plugin/packages/core/src/__tests__/ignore-generator.test.ts
...
Add to the end of `grasp-it-plugin/packages/core/src/index.ts`:
...
Run: `pnpm --filter @grasp-it/core build && pnpm --filter @grasp-it/core test -- --run`
...
**Files:**
- Modify: `grasp-it-plugin/agents/project-scanner.md`

- [ ] **Step 1: Read the current project-scanner.md**

Read `grasp-it-plugin/agents/project-scanner.md` to understand the current structure.
...
1. Check if `.grasp-it/.understandignore` exists in the project root. If so, read it.
...
```bash
git add grasp-it-plugin/agents/project-scanner.md
...
Read `grasp-it-plugin/skills/understand/SKILL.md` lines 22-80 to understand Phase 0.
...
1. Check if `$PROJECT_ROOT/.grasp-it/.understandignore` exists.
2. **If it does NOT exist**, generate a starter file:
   ...
   - Write the generated content to `$PROJECT_ROOT/.grasp-it/.understandignore`.
   - Report to the user:
     > "Generated `.grasp-it/.understandignore` with suggested exclusions based on your project structure. Please review it and uncomment any patterns you'd like to exclude from analysis. When ready, confirm to continue."
   - **Wait for user confirmation before proceeding.**
3. **If it already exists**, report:
   > "Found `.grasp-it/.understandignore`. Review it if needed, then confirm to continue."
   - **Wait for user confirmation before proceeding.**
...
Run: `pnpm --filter @grasp-it/skill build`
Expected: Clean build
...
Run:
```bash
ls grasp-it-plugin/packages/core/src/ignore-filter.ts grasp-it-plugin/packages/core/src/ignore-generator.ts
```
...
Run:
```bash
node -e "import('@grasp-it/core').then(m => { console.log('IgnoreFilter:', typeof m.createIgnoreFilter); console.log('Generator:', typeof m.generateStarterIgnoreFile); })"
```
