...
**Goal:** Make Grasp-It skills work across Codex, OpenClaw, OpenCode, and Cursor — same files everywhere, no build step.
...
**Files:**
- Move: `grasp-it-plugin/agents/project-scanner.md` → `grasp-it-plugin/skills/understand/project-scanner-prompt.md`
- Move: `grasp-it-plugin/agents/file-analyzer.md` → `grasp-it-plugin/skills/understand/file-analyzer-prompt.md`
- Move: `grasp-it-plugin/agents/architecture-analyzer.md` → `grasp-it-plugin/skills/understand/architecture-analyzer-prompt.md`
- Move: `grasp-it-plugin/agents/tour-builder.md` → `grasp-it-plugin/skills/understand/tour-builder-prompt.md`
- Move: `grasp-it-plugin/agents/graph-reviewer.md` → `grasp-it-plugin/skills/understand/graph-reviewer-prompt.md`
...
```bash
ls grasp-it-plugin/skills/understand/
```
...
```bash
git add -A grasp-it-plugin/agents/ grasp-it-plugin/skills/understand/
...
Read `grasp-it-plugin/skills/understand/SKILL.md` in full.
...
**Files:**
- Create: `grasp-it-plugin/agents/knowledge-graph-guide.md`
...
You are an expert on Grasp-It knowledge graphs. You help users navigate, query, and understand the `knowledge-graph.json` files produced by the `/understand` skill.

## What You Know

### Graph Location

The knowledge graph lives at `<project-root>/.grasp-it/knowledge-graph.json`. Metadata is at `<project-root>/.grasp-it/meta.json`.
...
```bash
git add grasp-it-plugin/agents/knowledge-graph-guide.md
...
**Files:**
- Move: `grasp-it-plugin/.codex/INSTALL.md` → `.codex/INSTALL.md`
- Move: `grasp-it-plugin/.opencode/INSTALL.md` → `.opencode/INSTALL.md`
- Move: `grasp-it-plugin/.openclaw/INSTALL.md` → `.openclaw/INSTALL.md`
- Delete: `grasp-it-plugin/.cursor/INSTALL.md` (replaced by `.cursor-plugin/plugin.json`)

**Step 1: Move the three platform directories to root**

```bash
cd /Users/yuxianglin/Desktop/opensource/Grasp-It
git mv grasp-it-plugin/.codex ./.codex
git mv grasp-it-plugin/.opencode ./.opencode
git mv grasp-it-plugin/.openclaw ./.openclaw
```

**Step 2: Delete .cursor/ (replaced by .cursor-plugin/ in Task 5)**

```bash
git rm -r grasp-it-plugin/.cursor/
```

**Step 3: Verify symlink paths are correct**

Read each INSTALL.md. The symlink paths should reference `grasp-it-plugin/skills` — this is still correct since the skills directory remains inside the plugin wrapper.
...
```json
{
  ...
  "version": "1.0.5",
  "author": { "name": "akimkelar" },
  "homepage": "https://github.com/akimkelar/Grasp-It",
  "repository": "https://github.com/akimkelar/Grasp-It",
  "license": "MIT",
  "keywords": ["codebase-analysis", "knowledge-graph", "architecture", "onboarding", "dashboard"],
  "skills": "./grasp-it-plugin/skills/",
  "agents": "./grasp-it-plugin/agents/"
}
```

Note: paths point into `grasp-it-plugin/` since the source stays nested.
...
The INSTALL.md files moved from `grasp-it-plugin/.codex/INSTALL.md` to `.codex/INSTALL.md`. Update all raw GitHub URLs:

```
OLD: .../refs/heads/main/grasp-it-plugin/.codex/INSTALL.md
NEW: .../refs/heads/main/.codex/INSTALL.md

OLD: .../refs/heads/main/grasp-it-plugin/.openclaw/INSTALL.md
NEW: .../refs/heads/main/.openclaw/INSTALL.md

OLD: .../refs/heads/main/grasp-it-plugin/.opencode/INSTALL.md
...
```bash
ls grasp-it-plugin/skills/understand/
ls grasp-it-plugin/agents/
ls grasp-it-plugin/packages/
```
...
```bash
ls grasp-it-plugin/.codex/ 2>/dev/null    # should fail
ls grasp-it-plugin/.cursor/ 2>/dev/null   # should fail
ls grasp-it-plugin/.opencode/ 2>/dev/null # should fail
ls grasp-it-plugin/.openclaw/ 2>/dev/null # should fail
```

**Step 4: Run tests**

```bash
pnpm --filter @grasp-it/core build && pnpm --filter @grasp-it/core test
```
...
Expected: `"source": "./grasp-it-plugin"` — unchanged, still correct.

**Step 6: Verify no stale raw GitHub URLs**

```bash
grep -r "grasp-it-plugin/\." README.md
```
