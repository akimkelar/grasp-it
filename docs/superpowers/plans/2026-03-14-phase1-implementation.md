...
```json
{
  ...
  "scripts": {
    "build": "pnpm -r build",
    "test": "vitest",
    "dev:dashboard": "pnpm --filter @grasp-it/dashboard dev",
    "lint": "eslint ."
  },
  "devDependencies": {
    "typescript": "^5.7.0",
...
Run: `cd /path/to/project && pnpm install && pnpm --filter @grasp-it/core build`
...
describe("Persistence", () => {
  ...
  it("saveGraph writes knowledge-graph.json", () => {
    ...
    const filePath = join(testDir, ".grasp-it", "knowledge-graph.json");
  ...
  it("saveMeta writes meta.json", () => {
    ...
    const filePath = join(testDir, ".grasp-it", "meta.json");
    expect(existsSync(filePath)).toBe(true);
  });
...
Run: `pnpm --filter @grasp-it/core add tree-sitter tree-sitter-javascript tree-sitter-typescript`
...
import type { KnowledgeGraph, GraphNode } from "@grasp-it/core";
...
// In production, this comes from .grasp-it/knowledge-graph.json
...
Run: `pnpm --filter @grasp-it/dashboard dev`
...
- Knowledge graph JSON lives in `.grasp-it/` directory of analyzed projects
```
...
2. **`pnpm --filter @grasp-it/core build`** — Compiles clean
3. **`pnpm --filter @grasp-it/core test`** — All tests pass (types, persistence, tree-sitter, graph builder, LLM analyzer)
