...
1. Read the knowledge graph file at `.grasp-it/knowledge-graph.json`
...
export function buildOnboardingGuide(graph: KnowledgeGraph): string {
  ...
  lines.push("---");
  lines.push("");
  lines.push(`*Generated by [Understand Anything](https://github.com/anthropics/grasp-it) from knowledge graph v${graph.version}*`);
  lines.push("");
...
1. Read the knowledge graph at `.grasp-it/knowledge-graph.json`
2. If it doesn't exist, tell the user to run `/understand` first
...
**Context:** The plugin registry from Task 4 manages runtime plugins, but we also need a way to discover and configure plugins from the project's `.grasp-it/` directory. This task adds a plugin configuration file schema and a discovery mechanism that scans for installed plugins and auto-registers them.
