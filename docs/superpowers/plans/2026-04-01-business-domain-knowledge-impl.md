...
Run: `cd grasp-it-plugin && pnpm --filter @grasp-it/core test -- --run src/__tests__/domain-types.test.ts`

Expected: FAIL — "domain" is not a valid NodeType enum value

- [ ] **Step 3: Add domain/flow/step to NodeType union**

In `grasp-it-plugin/packages/core/src/types.ts`, update the `NodeType` union (lines 1-5):
...
In `grasp-it-plugin/packages/core/src/schema.ts`:
...
Run: `cd grasp-it-plugin && pnpm --filter @grasp-it/core test -- --run`
...
**Files:**
- Modify: `grasp-it-plugin/packages/core/src/persistence/index.ts`
- Modify: `grasp-it-plugin/packages/core/src/index.ts`

- [ ] **Step 1: Write failing test for domain graph persistence**

```typescript
// grasp-it-plugin/packages/core/src/__tests__/domain-persistence.test.ts
...
Run: `cd grasp-it-plugin && pnpm --filter @grasp-it/core test -- --run src/__tests__/domain-persistence.test.ts`

Expected: FAIL — `saveDomainGraph` is not exported

- [ ] **Step 3: Implement saveDomainGraph and loadDomainGraph**

Add to `grasp-it-plugin/packages/core/src/persistence/index.ts` (after `loadConfig` function, before end of file):
...
Add to `grasp-it-plugin/packages/core/src/index.ts` (after the existing persistence re-exports on line 2):
...
```bash
git add grasp-it-plugin/packages/core/src/persistence/index.ts \
       grasp-it-plugin/packages/core/src/__tests__/domain-persistence.test.ts
...
**Files:**
- Modify: `grasp-it-plugin/packages/core/src/analyzer/normalize-graph.ts`

- [ ] **Step 1: Write failing test for domain ID normalization**

```typescript
// grasp-it-plugin/packages/core/src/__tests__/domain-normalize.test.ts
import { describe, it, expect } from "vitest";
...
Run: `cd grasp-it-plugin && pnpm --filter @grasp-it/core test -- --run src/__tests__/domain-normalize.test.ts`

Expected: FAIL — "domain" is not a valid prefix in `VALID_PREFIXES`

- [ ] **Step 3: Add domain prefixes to normalize-graph.ts**

In `grasp-it-plugin/packages/core/src/analyzer/normalize-graph.ts`:
...
```bash
git add grasp-it-plugin/packages/core/src/analyzer/normalize-graph.ts \
       grasp-it-plugin/packages/core/src/__tests__/domain-normalize.test.ts
...
In `grasp-it-plugin/packages/dashboard/src/store.ts`:
...
In `grasp-it-plugin/packages/dashboard/src/App.tsx`, add store selectors (after line 81):
...
```bash
git add grasp-it-plugin/packages/dashboard/src/App.tsx
...
**Files:**
- Create: `grasp-it-plugin/packages/dashboard/src/components/DomainClusterNode.tsx`

- [ ] **Step 1: Create the DomainClusterNode component**

```typescript
// grasp-it-plugin/packages/dashboard/src/components/DomainClusterNode.tsx
...
```bash
git add grasp-it-plugin/packages/dashboard/src/components/DomainClusterNode.tsx
...
**Files:**
- Create: `grasp-it-plugin/packages/dashboard/src/components/FlowNode.tsx`
- Create: `grasp-it-plugin/packages/dashboard/src/components/StepNode.tsx`

- [ ] **Step 1: Create the FlowNode component**

```typescript
// grasp-it-plugin/packages/dashboard/src/components/FlowNode.tsx
import { memo } from "react";
import { Handle, Position } from "@xyflow/react";
import type { Node, NodeProps } from "@xyflow/react";
import { useDashboardStore } from "../store";
...
```typescript
// grasp-it-plugin/packages/dashboard/src/components/StepNode.tsx
import { memo } from "react";
import { Handle, Position } from "@xyflow/react";
import type { Node, NodeProps } from "@xyflow/react";
import { useDashboardStore } from "../store";
...
```bash
git add grasp-it-plugin/packages/dashboard/src/components/FlowNode.tsx \
       grasp-it-plugin/packages/dashboard/src/components/StepNode.tsx
...
**Files:**
- Create: `grasp-it-plugin/packages/dashboard/src/components/DomainGraphView.tsx`
- Modify: `grasp-it-plugin/packages/dashboard/src/App.tsx`

- [ ] **Step 1: Create the DomainGraphView component**

```typescript
// grasp-it-plugin/packages/dashboard/src/components/DomainGraphView.tsx
...
import type { KnowledgeGraph, GraphNode } from "@grasp-it/core/types";
...
In `grasp-it-plugin/packages/dashboard/src/App.tsx`:
...
```bash
git add grasp-it-plugin/packages/dashboard/src/components/DomainGraphView.tsx \
       grasp-it-plugin/packages/dashboard/src/App.tsx
...
In `grasp-it-plugin/packages/dashboard/src/components/GraphView.tsx`, update `NODE_TYPE_TO_CATEGORY` (lines 53-59):
...
**Files:**
- Create: `grasp-it-plugin/agents/domain-analyzer.md`
...
```bash
git add grasp-it-plugin/agents/domain-analyzer.md
...
**Files:**
- Create: `grasp-it-plugin/skills/understand-domain/SKILL.md`

- [ ] **Step 1: Create the skill directory and SKILL.md**

```bash
mkdir -p grasp-it-plugin/skills/understand-domain
```
...
1. Check if `.grasp-it/knowledge-graph.json` exists in the current project
...
1. Run the preprocessing script bundled with this skill:
   ```
   python grasp-it-plugin/skills/understand-domain/extract-domain-context.py <project-root>
   ```
   This outputs `.grasp-it/intermediate/domain-context.json` containing:
...
1. Read `.grasp-it/knowledge-graph.json`
...
3. The agent writes its output to `.grasp-it/intermediate/domain-analysis.json`
...
4. Save to `.grasp-it/domain-graph.json`
5. Clean up `.grasp-it/intermediate/domain-analysis.json`
...
```bash
git add grasp-it-plugin/skills/understand-domain/SKILL.md
...
**Files:**
- Modify: `grasp-it-plugin/skills/understand-dashboard/SKILL.md`

- [ ] **Step 1: Read the existing understand-dashboard skill**

Read `grasp-it-plugin/skills/understand-dashboard/SKILL.md` to understand how the dashboard server is configured, then add `domain-graph.json` to the list of served files.

The dashboard server serves files from `.grasp-it/`. The domain graph file (`domain-graph.json`) needs to be served alongside `knowledge-graph.json` and `meta.json`.

Update the skill to mention that `domain-graph.json` should also be served if it exists. The exact change depends on how the server is configured in the skill — typically it serves the entire `.grasp-it/` directory, so `domain-graph.json` should be automatically available. Verify this is the case.
...
- `grasp-it-plugin/skills/understand-domain/SKILL.md`
- `grasp-it-plugin/skills/understand-domain/extract-domain-context.py`

### Modified Files
- `grasp-it-plugin/packages/core/src/types.ts` — 3 new node types, 3 new edge types, DomainMeta interface
- `grasp-it-plugin/packages/core/src/schema.ts` — Zod schemas + aliases for domain types
- `grasp-it-plugin/packages/core/src/persistence/index.ts` — saveDomainGraph/loadDomainGraph
- `grasp-it-plugin/packages/core/src/analyzer/normalize-graph.ts` — domain ID prefixes
- `grasp-it-plugin/packages/dashboard/src/store.ts` — viewMode, domainGraph, activeDomainId
- `grasp-it-plugin/packages/dashboard/src/App.tsx` — domain graph loading, view toggle, conditional rendering
- `grasp-it-plugin/packages/dashboard/src/components/NodeInfo.tsx` — domain-aware sidebar
- `grasp-it-plugin/packages/dashboard/src/components/GraphView.tsx` — NODE_TYPE_TO_CATEGORY update
