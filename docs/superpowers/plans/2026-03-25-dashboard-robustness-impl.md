...
**Files:**
- Modify: `grasp-it-plugin/packages/core/src/schema.ts:95-99`
...
**Files:**
- Modify: `grasp-it-plugin/packages/core/src/schema.ts:138-151`
- Test: `grasp-it-plugin/packages/core/src/__tests__/schema.test.ts`
...
**Files:**
- Modify: `grasp-it-plugin/packages/core/src/__tests__/schema.test.ts`
...
```bash
git add grasp-it-plugin/packages/core/src/__tests__/schema.test.ts
...
**Files:**
- Create: `grasp-it-plugin/packages/dashboard/src/components/WarningBanner.tsx`
    ...
    .then((data: unknown) => {
      ...
      if (result.success && result.data) {
        ...
        if (result.issues.length > 0) {
          ...
          if (autoCorrected.length > 0) console.warn(`[grasp-it] Auto-corrected ${autoCorrected.length} graph issues`);
          if (dropped.length > 0) console.error(`[grasp-it] Dropped ${dropped.length} broken graph items`);
        }
...
Run: `pnpm --filter @grasp-it/core build && pnpm --filter @grasp-it/dashboard build`
