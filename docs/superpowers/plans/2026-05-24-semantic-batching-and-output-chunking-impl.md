...
**Tech Stack:** Node.js ≥22 + pnpm ≥10, `graphology` + `graphology-communities-louvain` (new deps), `@grasp-it/core` TreeSitterPlugin (existing), Vitest for `.mjs` tests, Python `unittest` for `merge-batch-graphs.py` tests.
...
- `grasp-it-plugin/skills/understand/compute-batches.mjs` — Phase 1.5 script
- `grasp-it-plugin/skills/understand/test_compute_batches.test.mjs` — Vitest unit tests
- `grasp-it-plugin/skills/understand/test/fixtures/scan-result-3-cliques.json` — synthetic test fixture (3 disjoint import cliques)
- `grasp-it-plugin/skills/understand/test/fixtures/scan-result-large-community.json` — synthetic test fixture (50-node complete graph)
- `grasp-it-plugin/skills/understand/test/fixtures/scan-result-non-code.json` — synthetic test fixture (Dockerfile/CI/SQL groups)

**Modify:**

- `grasp-it-plugin/package.json` — add `graphology` + `graphology-communities-louvain` to `dependencies`
- `grasp-it-plugin/skills/understand/SKILL.md` — insert Phase 1.5; replace Phase 2 ANALYZE batching prose; replace Incremental update path
- `grasp-it-plugin/agents/file-analyzer.md` — add Cross-batch context (neighborMap) section; replace Writing Results with multi-part protocol
- `grasp-it-plugin/skills/understand/merge-batch-graphs.py` — multi-part stderr summary + missing-part warning
- `grasp-it-plugin/skills/understand/test_merge_batch_graphs.py` — new `TestMultiPart` class
- `grasp-it-plugin/package.json`, `grasp-it-plugin/.claude-plugin/plugin.json`, `.claude-plugin/plugin.json`, `.cursor-plugin/plugin.json`, `.copilot-plugin/plugin.json` — version bump (Task 16)
...
**Files:**
- Modify: `grasp-it-plugin/package.json`

- [ ] **Step 1: Add deps to package.json**

Edit `grasp-it-plugin/package.json` `dependencies` block:
...
```bash
git add grasp-it-plugin/package.json pnpm-lock.yaml
...
**Files:**
- Create: `grasp-it-plugin/skills/understand/compute-batches.mjs`
- Create: `grasp-it-plugin/skills/understand/test/fixtures/scan-result-3-cliques.json`
 ...
 * Output: <project-root>/.grasp-it/intermediate/batches.json
 */
...
```bash
mkdir -p /tmp/ua-prototype/.grasp-it/intermediate
cp grasp-it-plugin/skills/understand/test/fixtures/scan-result-3-cliques.json \
   /tmp/ua-prototype/.grasp-it/intermediate/scan-result.json
node grasp-it-plugin/skills/understand/compute-batches.mjs /tmp/ua-prototype
```
...
```bash
if [ -f .grasp-it/intermediate/scan-result.json ]; then
  node grasp-it-plugin/skills/understand/compute-batches.mjs "$(pwd)"
else
...
```bash
git add grasp-it-plugin/skills/understand/compute-batches.mjs \
        grasp-it-plugin/skills/understand/test/fixtures/scan-result-3-cliques.json
...
**Files:**
- Create: `grasp-it-plugin/skills/understand/test_compute_batches.test.mjs`
...
function setupProject(fixtureName) {
  ...
  const dest = join(root, '.grasp-it', 'intermediate', 'scan-result.json');
  writeFileSync(dest, readFileSync(fixturePath, 'utf-8'));
  return root;
}

function readBatches(projectRoot) {
  const p = join(projectRoot, '.grasp-it', 'intermediate', 'batches.json');
...
```bash
pnpm --filter @grasp-it/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "Louvain basic"
```
...
```javascript
  ...
  const outPath = join(projectRoot, '.grasp-it', 'intermediate', 'batches.json');
...
```bash
git add grasp-it-plugin/skills/understand/compute-batches.mjs \
        grasp-it-plugin/skills/understand/test_compute_batches.test.mjs
...
**Files:**
- Modify: `grasp-it-plugin/skills/understand/compute-batches.mjs`
- Modify: `grasp-it-plugin/skills/understand/test_compute_batches.test.mjs`
- Create: `grasp-it-plugin/skills/understand/test/fixtures/scan-result-large-community.json`

- [ ] **Step 1: Create large-community fixture (40-node complete graph in one community)**

Create `grasp-it-plugin/skills/understand/test/fixtures/scan-result-large-community.json`. Build programmatically once and commit the JSON:
...
" > grasp-it-plugin/skills/understand/test/fixtures/scan-result-large-community.json
```
...
```bash
pnpm --filter @grasp-it/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "size enforcement"
```
...
```bash
pnpm --filter @grasp-it/skill exec vitest run skills/understand/test_compute_batches.test.mjs
```
...
```bash
pnpm --filter @grasp-it/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "exports extraction"
```
...
- Modify: `grasp-it-plugin/skills/understand/compute-batches.mjs`
- Modify: `grasp-it-plugin/skills/understand/test_compute_batches.test.mjs`
- Create: `grasp-it-plugin/skills/understand/test/fixtures/scan-result-non-code.json`
...
```bash
pnpm --filter @grasp-it/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "non-code grouping"
```
...
git add grasp-it-plugin/skills/understand/compute-batches.mjs \
        grasp-it-plugin/skills/understand/test_compute_batches.test.mjs \
        grasp-it-plugin/skills/understand/test/fixtures/scan-result-non-code.json
...
```bash
pnpm --filter @grasp-it/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "neighborMap"
```
...
```bash
pnpm --filter @grasp-it/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "fallback"
```
...
```bash
pnpm --filter @grasp-it/skill exec vitest run skills/understand/test_compute_batches.test.mjs -t "changed-files"
```
...
**Files:**
- Modify: `grasp-it-plugin/agents/file-analyzer.md`

- [ ] **Step 1: Insert the new section**

In `grasp-it-plugin/agents/file-analyzer.md`, find the existing line:
```
...
```bash
grep -n "Cross-batch context (neighborMap)" grasp-it-plugin/agents/file-analyzer.md
grep -n "Step 1 — Prepare the input JSON" grasp-it-plugin/agents/file-analyzer.md
grep -n "Step 2 — Execute the bundled extraction script" grasp-it-plugin/agents/file-analyzer.md
```
...
```bash
git add grasp-it-plugin/agents/file-analyzer.md
...
In `grasp-it-plugin/agents/file-analyzer.md`, find the existing block (at approximately lines 467-475):
...
1. Write the JSON to: `<project-root>/.grasp-it/intermediate/batch-<batchIndex>.json`
...
**Step B — Decide split.**
- If `nodeCount ≤ 60` AND `edgeCount ≤ 120`: write ONE file to `.grasp-it/intermediate/batch-<batchIndex>.json`. Done. Skip to Step F.
...
Write part `k` (1-indexed) to `.grasp-it/intermediate/batch-<batchIndex>-part-<k>.json`. Each part is a valid GraphFragment: `{ "nodes": [...], "edges": [...] }`.
...
```bash
grep -n "Writing Results — single or multi-part" grasp-it-plugin/agents/file-analyzer.md
grep -n "Step A — Compute totals" grasp-it-plugin/agents/file-analyzer.md
grep -n "Step F — Respond" grasp-it-plugin/agents/file-analyzer.md
# Confirm old prose is gone:
! grep -n "After producing the JSON:" grasp-it-plugin/agents/file-analyzer.md
```
...
**Files:**
- Modify: `grasp-it-plugin/skills/understand/SKILL.md`

- [ ] **Step 1: Insert Phase 1.5 after Phase 1**

In `grasp-it-plugin/skills/understand/SKILL.md`, find the line:
```
## Phase 2 — ANALYZE
```
...
> Write output to: `$PROJECT_ROOT/.grasp-it/intermediate/batch-<batchIndex>.json`
>
...
This script reads all `batch-*.json` files from `$PROJECT_ROOT/.grasp-it/intermediate/`, then in one pass:
...
```bash
grep -n "Phase 1.5 — BATCH" grasp-it-plugin/skills/understand/SKILL.md
grep -n "Load \`.grasp-it/intermediate/batches.json\`" grasp-it-plugin/skills/understand/SKILL.md
grep -n "compute-batches.mjs" grasp-it-plugin/skills/understand/SKILL.md
# Confirm old prose is gone (each command should print "OK: ... absent"):
if grep -q "groups of \*\*20-30 files each\*\*" grasp-it-plugin/skills/understand/SKILL.md; then echo "FAIL: old batching prose still present"; else echo "OK: old batching prose absent"; fi
if grep -qF "Dockerfile + docker-compose.yml + .dockerignore → same batch" grasp-it-plugin/skills/understand/SKILL.md; then echo "FAIL: old non-code prose still present"; else echo "OK: old non-code prose absent"; fi
```
...
```bash
git add grasp-it-plugin/skills/understand/SKILL.md
...
**Files:**
- Modify: `grasp-it-plugin/skills/understand/merge-batch-graphs.py`
...
```bash
python3 -c "import ast; ast.parse(open('grasp-it-plugin/skills/understand/merge-batch-graphs.py').read())" && echo "OK"
```
...
```bash
cd grasp-it-plugin/skills/understand && python3 -m unittest test_merge_batch_graphs.py -v 2>&1 | tail -20
```
...
```bash
git add grasp-it-plugin/skills/understand/merge-batch-graphs.py
...
**Files:**
- Modify: `grasp-it-plugin/skills/understand/test_merge_batch_graphs.py`

- [ ] **Step 1: Append TestMultiPart class**

Append to `grasp-it-plugin/skills/understand/test_merge_batch_graphs.py`:
...
class TestMultiPart(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.intermediate = self.tmp / ".grasp-it" / "intermediate"
...
```bash
cd grasp-it-plugin/skills/understand && python3 -m unittest test_merge_batch_graphs.TestMultiPart -v
```
...
```bash
cd grasp-it-plugin/skills/understand && python3 -m unittest test_merge_batch_graphs -v 2>&1 | tail -5
```
...
```bash
git add grasp-it-plugin/skills/understand/test_merge_batch_graphs.py
...
```bash
INSTALLED_VERSION=$(ls ~/.claude/plugins/cache/grasp-it/grasp-it/ | head -1)
echo "Installed version: $INSTALLED_VERSION"
rm -rf ~/.claude/plugins/cache/grasp-it/grasp-it/$INSTALLED_VERSION
cp -R ./grasp-it-plugin ~/.claude/plugins/cache/grasp-it/grasp-it/$INSTALLED_VERSION
```
...
- At least one batch with > 60 nodes / > 120 edges triggers multi-part output (look in `.grasp-it/intermediate/` for any `batch-<i>-part-<k>.json` files)
...
jq '.algorithm, .totalFiles, .totalBatches, (.batches | length), [.batches[].files | length]' \
  .grasp-it/intermediate/batches.json 2>/dev/null \
...
Note: Phase 7 cleans up `.grasp-it/intermediate/` so this is best inspected mid-run, not after.
...
**Files:**
- Modify: `grasp-it-plugin/package.json`
- Modify: `grasp-it-plugin/.claude-plugin/plugin.json`
...
Current version is `2.7.4` (per `grasp-it-plugin/package.json` line 3). This PR adds a substantial feature (Phase 1.5 + multi-part output) — bump **minor**: `2.8.0`.
...
```bash
git add grasp-it-plugin/package.json \
        grasp-it-plugin/.claude-plugin/plugin.json \
        .claude-plugin/plugin.json \
        .cursor-plugin/plugin.json \
        .copilot-plugin/plugin.json
...
- [x] `pnpm --filter @grasp-it/core build`
- [x] `pnpm --filter @grasp-it/skill exec vitest run skills/understand/test_compute_batches.test.mjs` — all green
- [x] `cd grasp-it-plugin/skills/understand && python3 -m unittest test_merge_batch_graphs -v` — all green
