...
- Modify: `grasp-it-plugin/packages/core/src/types.ts` — add 5 node types, 6 edge types, `KnowledgeMeta` interface, `kind` field
- Modify: `grasp-it-plugin/packages/core/src/schema.ts` — add new types to Zod schemas, add aliases
- Modify: `grasp-it-plugin/packages/core/src/types.test.ts` — add tests for new types
- Test: `grasp-it-plugin/packages/core/src/__tests__/knowledge-schema.test.ts` — validation tests for knowledge-specific schema

### Dashboard changes
- Modify: `grasp-it-plugin/packages/dashboard/src/store.ts` — add knowledge node types, edge categories, `ViewMode`, node categories
- Modify: `grasp-it-plugin/packages/dashboard/src/components/CustomNode.tsx` — add colors for 5 new node types
- Modify: `grasp-it-plugin/packages/dashboard/src/components/NodeInfo.tsx` — add badge colors and edge labels for new types, add knowledge sidebar sections
- Modify: `grasp-it-plugin/packages/dashboard/src/components/ProjectOverview.tsx` — add knowledge-specific stats
- Modify: `grasp-it-plugin/packages/dashboard/src/index.css` — add CSS variables for 5 new node colors
- Modify: `grasp-it-plugin/packages/dashboard/src/App.tsx` — detect `kind` field, set view mode
- Create: `grasp-it-plugin/packages/dashboard/src/components/KnowledgeInfo.tsx` — knowledge-specific sidebar
- Create: `grasp-it-plugin/packages/dashboard/src/components/ReadingPanel.tsx` — full article reading overlay

### Skill & agent definitions
- Create: `grasp-it-plugin/skills/understand-knowledge/SKILL.md` — skill entry point
- Create: `grasp-it-plugin/skills/understand-knowledge/formats/obsidian.md`
- Create: `grasp-it-plugin/skills/understand-knowledge/formats/logseq.md`
- Create: `grasp-it-plugin/skills/understand-knowledge/formats/dendron.md`
- Create: `grasp-it-plugin/skills/understand-knowledge/formats/foam.md`
- Create: `grasp-it-plugin/skills/understand-knowledge/formats/karpathy.md`
- Create: `grasp-it-plugin/skills/understand-knowledge/formats/zettelkasten.md`
- Create: `grasp-it-plugin/skills/understand-knowledge/formats/plain.md`
- Create: `grasp-it-plugin/agents/knowledge-scanner.md`
- Create: `grasp-it-plugin/agents/format-detector.md`
- Create: `grasp-it-plugin/agents/article-analyzer.md`
- Create: `grasp-it-plugin/agents/relationship-builder.md`
...
**Files:**
- Modify: `grasp-it-plugin/packages/core/src/types.ts`

- [ ] **Step 1: Add knowledge node types to NodeType union**

In `grasp-it-plugin/packages/core/src/types.ts`, add the 5 knowledge types after the domain types:
...
```bash
git add grasp-it-plugin/packages/core/src/types.ts
...
**Files:**
- Modify: `grasp-it-plugin/packages/core/src/schema.ts`
- Create: `grasp-it-plugin/packages/core/src/__tests__/knowledge-schema.test.ts`

- [ ] **Step 1: Add knowledge edge types to EdgeTypeSchema**

In `grasp-it-plugin/packages/core/src/schema.ts`, update the `EdgeTypeSchema` z.enum to include the 6 new types:
...
Run: `pnpm --filter @grasp-it/core test -- --run src/__tests__/knowledge-schema.test.ts`
...
Run: `pnpm --filter @grasp-it/core test -- --run`
Expected: All existing tests pass, new knowledge tests pass

- [ ] **Step 7: Commit**

```bash
git add grasp-it-plugin/packages/core/src/schema.ts grasp-it-plugin/packages/core/src/__tests__/knowledge-schema.test.ts
...
**Files:**
- Modify: `grasp-it-plugin/packages/dashboard/src/index.css`

- [ ] **Step 1: Add CSS variables for 5 knowledge node types**

In `grasp-it-plugin/packages/dashboard/src/index.css`, add after the existing `--color-node-resource` line:
...
```bash
git add grasp-it-plugin/packages/dashboard/src/index.css
...
```bash
git add grasp-it-plugin/packages/dashboard/src/store.ts
...
**Files:**
- Modify: `grasp-it-plugin/packages/dashboard/src/components/CustomNode.tsx`
- Modify: `grasp-it-plugin/packages/dashboard/src/components/NodeInfo.tsx`
...
```bash
git add grasp-it-plugin/packages/dashboard/src/components/CustomNode.tsx grasp-it-plugin/packages/dashboard/src/components/NodeInfo.tsx
...
**Files:**
- Create: `grasp-it-plugin/packages/dashboard/src/components/KnowledgeInfo.tsx`
...
import type { GraphNode, GraphEdge, KnowledgeGraph } from "@grasp-it/core/types";
...
In `grasp-it-plugin/packages/dashboard/src/App.tsx`, find where the sidebar renders `NodeInfo` and add a condition: if `graph.kind === "knowledge"` and a node is selected, render `KnowledgeInfo` instead of `NodeInfo`.
...
```bash
git add grasp-it-plugin/packages/dashboard/src/components/KnowledgeInfo.tsx grasp-it-plugin/packages/dashboard/src/App.tsx
...
**Files:**
- Create: `grasp-it-plugin/packages/dashboard/src/components/ReadingPanel.tsx`
- Modify: `grasp-it-plugin/packages/dashboard/src/App.tsx`
...
```bash
git add grasp-it-plugin/packages/dashboard/src/components/ReadingPanel.tsx grasp-it-plugin/packages/dashboard/src/App.tsx
...
- Modify: `grasp-it-plugin/packages/dashboard/src/utils/layout.ts` (if direction isn't already configurable)
...
```bash
git add grasp-it-plugin/packages/dashboard/src/components/GraphView.tsx
...
5. Write results to `$PROJECT_ROOT/.grasp-it/intermediate/knowledge-manifest.json`
...
- Write output to `.grasp-it/intermediate/knowledge-manifest.json`
```
...
Read `.grasp-it/intermediate/knowledge-manifest.json` produced by the knowledge-scanner.
...
Write to `.grasp-it/intermediate/format-detection.json`:
...
Write per-batch results to `.grasp-it/intermediate/article-batch-<N>.json`:
...
Read all `article-batch-*.json` files from `.grasp-it/intermediate/`. Merge all nodes and edges.
...
Write to `.grasp-it/intermediate/relationships.json`:
...
```bash
git add grasp-it-plugin/agents/knowledge-scanner.md grasp-it-plugin/agents/format-detector.md grasp-it-plugin/agents/article-analyzer.md grasp-it-plugin/agents/relationship-builder.md
...
Research Obsidian's official docs (https://help.obsidian.md/) and create `grasp-it-plugin/skills/understand-knowledge/formats/obsidian.md`:
...
Research Logseq docs (https://docs.logseq.com/) and create `grasp-it-plugin/skills/understand-knowledge/formats/logseq.md`:
...
Research Dendron wiki (https://wiki.dendron.so/) and create `grasp-it-plugin/skills/understand-knowledge/formats/dendron.md`:
...
Research Foam docs (https://foambubble.github.io/foam/) and create `grasp-it-plugin/skills/understand-knowledge/formats/foam.md`:
...
Research Karpathy's gist (https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) and create `grasp-it-plugin/skills/understand-knowledge/formats/karpathy.md`:
...
Research zettelkasten.de and create `grasp-it-plugin/skills/understand-knowledge/formats/zettelkasten.md`:
...
```bash
git add grasp-it-plugin/skills/understand-knowledge/formats/
...
**Files:**
- Create: `grasp-it-plugin/skills/understand-knowledge/SKILL.md`
...
2. Create `.grasp-it/` and `.grasp-it/intermediate/` directories if they don't exist
3. If `--ingest` flag is present:
   - Verify `.grasp-it/knowledge-graph.json` exists (error if not — must run full scan first)
...
Wait for the agent to write `.grasp-it/intermediate/knowledge-manifest.json`.
...
Wait for `.grasp-it/intermediate/format-detection.json`.
...
Wait for `.grasp-it/intermediate/relationships.json`.
...
1. Write `.grasp-it/knowledge-graph.json`
2. Write `.grasp-it/meta.json`:
   ```json
   {
   ...
   ```
3. Clean up `.grasp-it/intermediate/` directory
...
```bash
git add grasp-it-plugin/skills/understand-knowledge/SKILL.md
...
Run: `head -5 grasp-it-plugin/skills/understand-knowledge/SKILL.md`
Expected: Valid `---` delimited YAML with name, description, argument-hint

- [ ] **Step 6: Verify all agents are present**

Run: `ls grasp-it-plugin/agents/ | grep knowledge\|format\|article\|relationship`
...
Run: `ls grasp-it-plugin/skills/understand-knowledge/formats/`
