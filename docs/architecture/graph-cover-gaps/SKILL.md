---
name: graph-cover-gaps
description: Use when the user wants to add missing knowledge to the configured Neo4j graph after an investigation or for a named topic, for example `/graph-cover-gaps notification settings`. The main agent may pass conclusions or evidence hints, but a smaller sub-agent must verify the current graph state itself, determine the real gap, and produce a dated incremental Cypher refresh artifact.
allowed-tools: Bash
---
...
- when the user gives a focused topic and wants the graph enriched for it, for example `/graph-cover-gaps notification settings`
...
Do not hardcode `.codex/graph` or `.codex/feature-graph` up front.
Discover which documentation line matches the configured database.
...
Use the label families to choose the matching repo docs:

- if the graph contains labels such as `Domain`, `Feature`, `SubFeature`, `Entity`, `EntryPoint`, `ServiceMethod`, or `Evidence`, treat it as the feature graph and read:
  - `.codex/feature-graph/README.md`
  - `.codex/feature-graph/architecture.md`
  - `.codex/feature-graph/seeding-rules.md`
  - `.codex/feature-graph/outdating-rules.md`
- if the graph contains labels such as `FeatureArea`, `BusinessConcept`, `CodeEvidence`, or `DataArtifact` without the feature-graph business layer, treat it as the older graph line and read:
  - `.codex/graph/README.md`
  - `.codex/graph/architecture.md`
  - `.codex/graph/seeding-rules.md`
  - `.codex/graph/outdating-rules.md`

If the live database looks mixed or ambiguous:

- do not spend time globally classifying the whole database
- find the closest existing nodes for the target topic
- follow the label family and artifact lineage already used in that local neighborhood
- place the update artifact alongside the nearest matching topic artifacts
- if that still remains ambiguous, choose the narrower update plan and report the ambiguity instead of broadening the investigation
...
- do not re-open both graph-line doc sets unless the sanity check directly contradicts the provided context
...
- the likely graph line and update folder, if already known
...
4. write one dated incremental refresh artifact in the doc-appropriate updates folder
...
Start broad with the topic terms:
...
- one missing `Feature` or `SubFeature`
- missing `Operation` nodes
- missing `BusinessRule`, `Constraint`, `Risk`, `Impact`, or `Context` nodes
...
Write one dated incremental `.cypher` file in the updates location defined by the matching graph docs.
Use the docs for the chosen graph line to determine the correct root folder.
...
- `.codex/feature-graph/updates/2026-05-12-notification-settings-refresh.cypher`

### Cypher writing rules

- prefer `MERGE` by stable `key` for business nodes
- use `SET` to refresh metadata fields and summaries
- follow relationship directions from the matching graph architecture and seeding rules
...
MERGE (n:SomeLabel {key: 'stable.key'})
...
```cypher
MATCH (a:SomeLabel {key: 'a.key'}),
      (b:OtherLabel {key: 'b.key'})
...
- which graph docs it matched and used
