...
- **Knowledge subgraph** (`kind: "knowledge"`) — business and product knowledge extracted by LLM from codebase signals and concept plan sessions. Knowledge nodes carry a `source` property (`"code-analysis"` | `"concept"` | `"wiki"`) that records which skill produced them, distinguishing implemented facts from specialist-described intent.
...
Concept nodes (`source: "concept"`) carry `generatedAt` and `author` — they have no `sourceCommit` or `sourceFiles`. The `author` field records the name or role of the specialist who provided the knowledge.
...
- Source attribution: `author` traces concept plan knowledge back to its specialist
...
Generated by domain-analyzer agent (`/grasp-domain`) and concept plan (`/grasp-concept`).

All knowledge nodes carry `generatedAt` (ISO 8601 timestamp). Nodes from code analysis additionally carry `sourceCommit` and `sourceFiles`; concept nodes do not.
...
**Concept nodes** (`source: "concept"`) — produced by `/grasp-concept`. The same node types can appear; only the `source` property differs. Concept nodes do NOT carry `sourceCommit` or `sourceFiles`.

#### Node Reuse Guidelines for Concept Plans

During a concept plan, the skill must check the graph before creating new nodes:
...
| `Claim` | **Concept-specific.** Assertions from the concept plan (`confidence: tentative\|agreed`). |
