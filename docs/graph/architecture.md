...
Every knowledge node carries two provenance properties:

| Property | Type | Description |
|----------|------|-------------|
| `generatedAt` | ISO 8601 timestamp | When the node was created or last refreshed |
| `sourceCommit` | string (git hash) | Git commit hash at which this node was derived from code. Present on nodes produced by code analysis (`source: "code-analysis"`). Absent on nodes derived purely from PO interviews (`source: "interview"`). |

These properties live on individual nodes, not just at the project level, enabling:
- Per-node staleness detection: compare `sourceCommit` against current HEAD to determine if a node's underlying code has changed
- Prioritization: `generatedAt` allows re-analysis to proceed oldest-first
- Auditability: trace when each piece of knowledge was extracted and from which commit
...
|-------|-------------|-----------|-----------------|
| `Domain` | Product domain or area | `domain:<kebab-name>` | `generatedAt`, `sourceCommit` |
| `Feature` | Named product capability | `feature:<kebab-name>` | `generatedAt`, `sourceCommit` |
| `Operation` | A meaningful action within a feature | `operation:<kebab-name>` | `generatedAt`, `sourceCommit` |
| `Actor` | User role or system agent | `actor:<kebab-name>` | `generatedAt` |
| `BusinessRule` | High-level business policy | `business-rule:<kebab-name>` | `generatedAt`, `sourceCommit` |
| `Entity` | Named business object | `entity:<kebab-name>` | `generatedAt`, `sourceCommit` |
| `Risk` | Potential negative outcome (code-visible hazard or specialist-identified risk) | `risk:<kebab-name>` | `generatedAt`, `sourceCommit` |
| `Constraint` | Technical invariant or access condition | `constraint:<kebab-name>` | `generatedAt`, `sourceCommit` |

**PO Interview Layer** — produced by `/grasp-requirements` (`source: "interview"`); `Risk` and `Constraint` also appear here when identified during PO interviews:

| Label | Description | ID format | Key properties |
|-------|-------------|-----------|-----------------|
| `Decision` | Resolved question or commitment | `decision:<kebab-name>` | `generatedAt` |
| `Concept` | Key abstraction named by the specialist | `concept:<kebab-name>` | `generatedAt` |
| `Claim` | Assertion from the interview (`confidence: tentative\|agreed`) | `claim:<short-uuid>` | `generatedAt` |
| `Risk` | Potential negative outcome (specialist-identified risk) | `risk:<kebab-name>` | `generatedAt` |
| `Constraint` | Technical invariant or access condition stated by specialist | `constraint:<kebab-name>` | `generatedAt` |
