...
9. **Apply `.graspignore` exclusions** (same semantics as `/understand` Step 2.5 in `agents/project-scanner.md`).
   ...
   1. If neither `$PROJECT_ROOT/.grasp-it/.graspignore` nor `$PROJECT_ROOT/.graspignore` exists, the step 7 extension filter is sufficient — skip to Phase 1.

   2. Write the step 7 file list to `$PROJECT_ROOT/.grasp-it/intermediate/changed-files-pre.json` as a JSON array of relative paths.

   3. Resolve `$PLUGIN_ROOT`:
      - Use `$CLAUDE_PLUGIN_ROOT` if set (Claude Code's hook context sets this).
      - Otherwise try `$HOME/.grasp-it-plugin`.
      - Validate the chosen candidate by checking `$candidate/packages/core/dist/ignore-filter.js` exists.
      - If neither resolves: report "Cannot locate plugin install at `$CLAUDE_PLUGIN_ROOT` or `$HOME/.grasp-it-plugin`; auto-update aborted. Run `/understand` to re-baseline." and **STOP**. Do **not** silently skip — silent skip reproduces issue #153.

   4. Write `$PROJECT_ROOT/.grasp-it/intermediate/ignore-filter.mjs`:
      ```javascript
      ...
      writeFileSync(
        path.join(PROJECT_ROOT, '.grasp-it/intermediate/changed-files.json'),
        JSON.stringify({ kept, removed, total: input.length }, null, 2),
      );
      console.log(`.graspignore: kept ${kept.length}/${input.length} (removed ${removed})`);
      ```

   5. Run it:
      ```bash
      node $PROJECT_ROOT/.grasp-it/intermediate/ignore-filter.mjs \
        "$PLUGIN_ROOT" \
        $PROJECT_ROOT/.grasp-it/intermediate/changed-files-pre.json
      ```
...
1. Write and execute a Node.js script (`$PROJECT_ROOT/.grasp-it/intermediate/fingerprint-check.mjs`):

```javascript
// The script should:
// 1. Read fingerprints.json from .grasp-it/fingerprints.json
...
// 6. Write result to .grasp-it/intermediate/change-analysis.json
```
...
2. Read `.grasp-it/intermediate/change-analysis.json`.
...
3. For each batch, dispatch a subagent using the `file-analyzer` agent definition (at `agents/file-analyzer.md`). Append:
   ...
   > Languages: `<languages>`
   > Batch index: `1`
   > Write output to: `$PROJECT_ROOT/.grasp-it/intermediate/batch-1.json`
   >
...
2. **Update fingerprints (LOAD-PATCH-SAVE, not OVERWRITE).**
   ...
   const fpPath = path.join(PROJECT_ROOT, '.grasp-it', 'fingerprints.json');
