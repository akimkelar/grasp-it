...
```bash
node $PROJECT_ROOT/.grasp-it/tmp/ua-graph-validate.js "<graph-file-path>" "$PROJECT_ROOT/.grasp-it/tmp/ua-review-results.json"
```
...
After the script completes, read `$PROJECT_ROOT/.grasp-it/tmp/ua-review-results.json`. Do NOT re-read the original graph file -- trust the script's results entirely.
...
1. Write the JSON to: `<project-root>/.grasp-it/intermediate/review.json`
