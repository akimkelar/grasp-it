...
```bash
cat > $PROJECT_ROOT/.grasp-it/tmp/ua-tour-input.json << 'ENDJSON'
{
...
```bash
node $PROJECT_ROOT/.grasp-it/tmp/ua-tour-analyze.js $PROJECT_ROOT/.grasp-it/tmp/ua-tour-input.json $PROJECT_ROOT/.grasp-it/tmp/ua-tour-results.json
```
...
After the script completes, read `$PROJECT_ROOT/.grasp-it/tmp/ua-tour-results.json`. Use the structural analysis as your primary guide for designing the tour. Do NOT re-read source files or re-analyze the graph -- trust the script's results entirely.
...
[
  {
    ...
    "description": "Start with README.md to grasp the project's purpose, architecture, and how to get started. This document outlines the main components and their relationships, providing a roadmap for the tour ahead.",
...
1. Write the JSON array to: `<project-root>/.grasp-it/intermediate/tour.json`
